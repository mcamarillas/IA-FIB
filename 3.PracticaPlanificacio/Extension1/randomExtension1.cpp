...
void header(int nh, int nr, ofstream &salida){
    salida << "(define (problem problemaExtension1)" << endl
    << " (:domain dominioExtension1)" << endl << endl
    << " (:objects" << endl << "   ";
    cout << nh << " " << nr << endl;
    for(int i = 1; i <= nh; ++i) {
      string str;
    ...
    salida << "   (= (reservasNoAsignadadas) 0)" << endl;
    srand(time(NULL));
    for(int i = 1; i <= nh; ++i) {
      int np = 1 + rand() % (4);
...
void ending(ofstream &salida) {
    ...
    << " )" << endl
    << "(:metric minimize (reservasNoAsignadadas))" << endl
    << ")" << endl;
}

int main(int argc, char **argv) {
   ofstream salida;
   salida.open("problemaExtension1.pddl");
   int nr = atoi(argv[2]);
   int nh = atoi(argv[1]);
   header(nh, nr, salida);
   body(nr, salida);
