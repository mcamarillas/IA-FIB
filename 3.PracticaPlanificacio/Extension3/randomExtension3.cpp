...
void header(int nh, int nr, ofstream &salida){
    salida << "(define (problem problemaExtension3)" << endl
    << " (:domain dominioExtension3)" << endl << endl
    << " (:objects" << endl << "   ";
    cout << nh << " " << nr << endl;
    for(int i = 1; i <= nh; ++i) {
      string str;
...
void ending(ofstream &salida, int nr) {
    salida << " )" << endl
    << " (:goal" << endl
    << "   (forall (?res - reserva) (resuelta?res))" << endl
    << " )" << endl
    << "   (:metric minimize (+ (* (reservasNoAsignadas) " << nr*3 << ") (plazasMalgastadas)))" << endl
    << ")" << endl;
}

int main(int argc, char **argv) {
   ofstream salida;
   salida.open("problemaExtension3.pddl");
   int nr = atoi(argv[2]);
   int nh = atoi(argv[1]);
   header(nh, nr, salida);
   body(nr, salida);
